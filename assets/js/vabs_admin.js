...
var url = vabs_ajax_obj.url + 'handler.php';

var trigger_one = document.querySelectorAll('input[name="page"]');
var step_two = document.getElementById('step_two');
var last_step = document.getElementById('last_step');
var final = document.getElementById('final');
var output_element = document.getElementById('vabs_shortcode_output_element');
var languages = document.querySelector('.vabs__formlist');
var langNav = document.querySelector('.navigation__links');
var links = document.querySelectorAll('.navigation__links--link');
var saveOptions = document.getElementById('save_main_options');
...
fetch('https://api.github.com/repos/uwehornnet/wpvabs/releases', {
	...
	if (version < parseFloat(response[0].tag_name.replace('v', ''))) {
		...
		template.querySelectorAll('button').forEach(function (button) {
...
if (saveOptions) {
	saveOptions.addEventListener('click', function (event) {
		event.preventDefault();
		var apiToken = document.querySelector('.vabs input[name="api_token"]').value;
...
function output() {
	outputData.courses = [];
	var courseInputList = document.querySelectorAll('input[name="course"]');
	courseInputList = Array.from(courseInputList).map(function (course) {
		if (course.checked) {
...
function ApiCall(method, data) {
	...
	if (method === 'get_all_courses') answer = groupBy(JSON.parse(answer), 'kurs_gruppen_name');

	return answer;
}

function getAllCourses() {
	var value = ApiCall('get_all_courses', {});
	appendElements(last_step, value);
	openElement(last_step);
}

function getCourseGroups() {
	var value = ApiCall('get_all_course_groups', {});
	...
	if (data.constructor === Object) {
		for (var key in data) {
			...
			for (var i = 0; i < data[key].length; i++) {
				...
				var desc = course.kurz_beschreibung ? course.kurz_beschreibung : '';
		...
		for (var i = 0; i < data.length; i++) {
			...
			var _desc = course.kurz_beschreibung ? course.kurz_beschreibung : '';
			label.innerHTML = '<input type="radio" name="course" class="from__field--radio" value="' + course.id + '"><span><strong>' + course.name + '</strong><small>' + _desc + '</small></span>';
